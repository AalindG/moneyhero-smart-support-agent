...
export default function App() {
  ...
  // 'idle' | 'confirm' | 'loading' | { ticketId, message } | { error }
  ...
  const handleEscalate = async () => {
    if (escalateState === 'confirm') {
      ...
      try {
        ...
        setEscalateState({ ticketId: res.ticketId, message: res.message })
      } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50">
      {/* Header */}
      <header className="bg-slate-900 text-white px-5 py-3.5 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5 leading-none">Smart Support Agent</p>
          </div>
        </div>
      ...
      {escalateState === 'confirm' && (
        <div className="bg-amber-50 border-b border-amber-200 px-5 py-2.5 flex items-center justify-between flex-shrink-0">
          <p className="text-xs text-amber-800 font-medium">
            Connect you with a human advisor?
          </p>
          <div className="flex items-center gap-3">
            <button
      ...
      {escalateState === 'loading' && (
        <div className="bg-blue-50 border-b border-blue-200 px-5 py-2.5 flex items-center gap-2 flex-shrink-0">
      ...
      {escalateState?.ticketId && (
        <div className="bg-emerald-50 border-b border-emerald-200 px-5 py-2.5 flex items-center justify-between flex-shrink-0">
          <p className="text-xs text-emerald-800">
            Ticket{' '}
            <span className="font-mono font-semibold">{escalateState.ticketId}</span> created.
            An advisor will contact you shortly.
          </p>
          <button
            onClick={() => setEscalateState('idle')}
            className="text-emerald-600 hover:text-emerald-800 ml-3 flex-shrink-0"
          >
      ...
      {escalateState?.error && (
        <div className="bg-red-50 border-b border-red-200 px-5 py-2.5 flex items-center justify-between flex-shrink-0">
